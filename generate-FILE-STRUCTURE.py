...
def should_exclude(
    ...
    """
    # Check 0: File extension (only for files)
    if not is_directory:
...
def build_tree_entries(
    ...
    """
    Recursively build a list of (tree_line, is_directory) tuples for the given path.
    ...
    Returns:
        List[tuple]: List of (line_string, is_dir) for tree rendering.
    """
...
def print_colored_console_tree(tree_entries: List[tuple], root_name: str):
    ...
    for line, is_dir in tree_entries:
...
def main():
    ...
    Steps:
      ...
      3. Load hardcoded exclusions (names, globs, extensions).
      4. Load .gitignore from project root (if exists).
      5. Build tree for target directory (with exclusions).
