...
void OrganicTextScene::drawGui() {
  ...
  // Session
  ot.gui.setPosition(SURFING__OFXGUI__PAD_TO_WINDOW_BORDERS + ot.gui.getWidth() +
                     SURFING__OFXGUI__PAD_BETWEEN_PANELS,
                     ofGetHeight() - ot.gui.getHeight() -
                     SURFING__OFXGUI__PAD_TO_WINDOW_BORDERS); //bottom left 2nd column
}
...
void OrganicTextScene::drawHelpDistribution() {
  ...
  s += "< >            Browse Presets";
  s += "\n";
  std::string sp = ofToString(slideshow.bEnabled_.get() ? "Slideshow Pause" :
                              "Slideshow Play");
  s += "P              " + sp;
  s += "\n";
  ...
  s += "\n";
  s += "PRESET         " + ofToString(pm.getPresetIndex()) + " / " +
       ofToString(pm.getPresetIndexLast());
  s += "\n";
